  ...
  return async () => {
    ...
    */
    ...
    const mbrTxn = await algorand.transactions.payment({
      ...
      extraFee: new AlgoAmount({ microAlgos: 1_000 }),
    })
    await nftmClient.bootstrap({ asset: assetBeingSold, unitaryPrice, mbrPay: mbrTxn })
    // 문제 7 끝
  ...
  return async () => {
    ...
    */
    ...
    try {
      ...
      // 문제 8 시작
      await nftmClient.compose().addTransaction(assetOptInTxn).execute()
      // 문제 8 끝
